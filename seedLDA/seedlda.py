...
class SeedLda:
    ...
    def seedLda(self):
        ...
        print('########storing model#########')
        # 第六步，存储模型
        with open(os.path.join(self.outputDir, "seedlda.model"), 'wb') as f:
            pickle.dump(model, f)
        # 第七步，输出每个主题下的词
        ...
        with open(os.path.join(self.outputDir, "topic_word.txt"), "a", encoding="utf-8") as f:
            ...
            for i, topic_dist in enumerate(topic_word, start=0):
                ...
                for word_id in top_n_word_id:
                    word = vocab[word_id]
                    p_word = topic_dist[word_id]
                    word_freqs[word] = p_word
                    line += (word + ":" + str(p_word) + "\t")
                    # top_words.append((vocab[word_id], topic_dist[word_id]))
                f.write(line + "\n\n")
                # self.get_cloud(word_freqs, i)
        ...
        with open(os.path.join(self.outputDir, "doc_topic.txt"), "a", encoding="utf-8") as f:
            doc_topic_dist = model.transform(doc_word)
            data_num = len(doc_topic_dist)
            print('数据条数：{}'.format(data_num))
            for i in range(0, data_num):
                line = "Document " + str(i) + ":"
                for p_topic in doc_topic_dist[i]:
                    line += ("\t" + str(p_topic))
                f.write(line + "\n")
                # print("top topic: {} Document: {}".format(doc_topic[i].argmax(), docs[i]))
...
if __name__ == '__main__':
    ...
    outputDir: 存储结果的文件夹
    """
    processed_corpus = "data/oculus/reviews_processing.txt"
    K = 50
    n_top_words = 15
    iterationNum = 1000
    seed_topic_list = [
